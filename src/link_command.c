...
bool ra_link_collect(struct ra_link_collector *collector,
    ...
    if (digit && digit != '#') {
        ...
        finish =
            ra_link_command_parse(mappings, count, collector->digits, &command) && !*command.node;
    }
    if (!finish) {
        return false;
    }
...
/** @brief Distinguish commands taking a destination from node-free status/actions.
