...
/** Product callback invoked synchronously by one radio exchange. */
typedef uint32_t (*rptadv_radio_render_v1)(void *context, uint32_t receiving, float *samples,
                                           size_t sample_count);
 ...
 * call and are never retained. PCM is aligned normalized F32 with exactly the stated
 * count. Radio render returns nonzero to key. Peer events are 1 text bytes, 2 one
 * DTMF byte, or 3 normalized F32 PCM. Directory methods are 0 both, 1 DNS, 2 file.
 */
struct rptadv_host_services_v1 {
    ...
    /** Open one uniquely owned radio. */
    int (*radio_open)(void *context, const char *name, size_t name_length, size_t maximum_frames,
                      void **radio);
    /** Wait briefly for radio work. */
    int (*radio_ready)(void *context, void *radio);
    /** Exchange one radio frame through the supplied renderer. */
    int (*radio_exchange)(void *context, void *radio, uint64_t now_ms,
                          rptadv_radio_render_v1 render, void *render_context);
    /** Destroy one uniquely owned radio. */
...
struct rptadv_product_descriptor_v1 {
    ...
    int (*start)(const struct rptadv_host_services_v1 *host,
